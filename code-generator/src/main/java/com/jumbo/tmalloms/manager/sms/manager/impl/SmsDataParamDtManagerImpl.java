package com.jumbo.tmalloms.manager.sms.manager.impl;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import com.jumbo.dao.sms.SmsDataParamDtDao;
import com.jumbo.tmalloms.manager.sms.manager.SmsDataParamDtManager;
import com.jumbo.tmalloms.manager.sms.vo.SmsDataParamDtVo;
import com.jumbo.tmalloms.manager.BaseManagerImpl;

/**
 * @author hailiang.jiang
 * @date 2015年09月25日 下午13:33:58
 */
@Service("smsDataParamDtManager")
@Transactional
public class SmsDataParamDtManagerImpl extends BaseManagerImpl implements SmsDataParamDtManager {
	private static final long serialVersionUID = 1L;
	
	@Autowired
	private SmsDataParamDtDao smsDataParamDtDao;

	@Override
	public SmsDataParamDtVo initPage(Long ouId) {
		// TODO Auto-generated method stub
		return null;
	}

}
